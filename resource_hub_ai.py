...
def extract_info_with_ai(raw_text=None, image_bytes=None):
    ...
    try:
        ...
        system_prompt = """
        你是一個專業的防災資源調度員。請判斷輸入內容是「需求 (Demand)」還是「供給 (Supply)」，並萃取為標準 JSON。
        【地理指示】請利用台灣地理知識，為提取出的地點推算大約的經緯度(lat與lon，浮點數)。若無法推算請填 23.5, 121.0。
        ...
        請嚴格遵守以下 JSON 格式輸出：
        {
          ...
          "data": {
              ...
              "category": "物資或機具類別",
              "item": "具體物品",
              "qty": 數量 (數字，若無具體數字請填 1),
              "urgency": 緊急度 (1至5的整數，5最緊急),
              "lat": 緯度浮點數,
              "lon": 經度浮點數
          }
        }
        ...
        if image_bytes:
            ...
            messages.append({
                "role": "user",
                "content": [
                    {"type": "text", "text": "請分析這張災情照片並萃取JSON：" + str(raw_text)},
    ...
    with col_in:
        ...
        if st.button("🧠 啟動多模態解析", type="primary"):
            with st.spinner("Llama-3 Vision 解析中..."):
                ...
                result = extract_info_with_ai(raw_text=text_to_send, image_bytes=img_bytes)
                
                if "error" in result: st.error("解析錯誤！")
                else:
